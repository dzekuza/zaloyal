-- Migration: Fix social accounts schema and authentication
-- This migration applies the final cleanup and fixes authentication issues
...
-- Step 8: Update increment_user_xp function to work with email
CREATE OR REPLACE FUNCTION public.increment_user_xp(user_email text, xp_amount integer)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
BEGIN
    ...
    SET 
        ...
        level = GREATEST(1, FLOOR((total_xp + xp_amount) / 100) + 1),
        updated_at = NOW()
