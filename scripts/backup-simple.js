...
async function simpleBackup() {
  ...
  try {
    ...
    // 6. Save to file
    const fs = require('fs');
    const backupFileName = `simple-backup-${new Date().toISOString().split('T')[0]}.json`;
    const backupPath = `scripts/${backupFileName}`;
    
    fs.writeFileSync(backupPath, JSON.stringify(backup, null, 2));
    
    console.log(`\n✅ Simple backup completed!`);
    ...
    // 7. Create SQL file
    const sqlFileName = `simple-backup-${new Date().toISOString().split('T')[0]}.sql`;
    const sqlPath = `scripts/${sqlFileName}`;
    
    let sqlContent = `-- Simple Database Backup - ${new Date().toISOString()}\n\n`;
